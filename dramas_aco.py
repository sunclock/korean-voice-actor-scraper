...
urls = {
    ...
    "complete_2": "https://audiocomics.kr/audiodrama/complete?per_page=2",
    "complete_3": "https://audiocomics.kr/audiodrama/complete?per_page=3",
    "complete_4": "https://audiocomics.kr/audiodrama/complete?per_page=4",
    "complete_5": "https://audiocomics.kr/audiodrama/complete?per_page=5",
...
def get_drama_list(html):
    ...
    dramas = []
    for drama in html:
        title = drama.find('h3', 'subjet').find('a').string
        url = drama.find('h3', 'subjet').find('a').get('href')
        ...
        writer = drama.find('div', 'writer').find('a').string
        if (title == '연애수업/연애사업'):
            title = '연애수업, 연애사업'
        if (production):
        ...
        voice_actor = drama.find(
            'div', 'voice_actor').string.replace('\n', '').replace('\t', '').replace('\r', '').split(',')
        for actor in voice_actor:
            actor = actor.strip()
        dramas.append({
            'title': title,
            'url': url,
